...
import polygo
from polygo.utils import load_data_and_labels, load_glove
...
class TestWrapper(unittest.TestCase):
    ...
    def test_save_and_load(self):
        ...
        model = polygo.Sequence.load(weights_file, params_file, preprocessor_file)
    ...
    def test_train_vocab_init(self):
        ...
        for words in np.r_[self.x_train, self.x_test, self.x_test]:
            for word in words:
                vocab.add(word)
        model = polygo.Sequence(initial_vocab=vocab, embeddings=self.embeddings)
        model.fit(self.x_train, self.y_train, self.x_test, self.y_test)
